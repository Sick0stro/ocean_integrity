...
if uploaded_file:
    ...
    city_counts = df.groupby("city")["invoice_weight_mt"].sum().reset_index()
    city_counts = city_counts[city_counts["city"] != ""]  # Filter out empty cities
    if not city_counts.empty:
        fig = px.bar(
            city_counts,
            x="city",
            y="invoice_weight_mt",
            title="Total Weight by City (MT)",
            text_auto=True,
        ...
        # City compliance analysis
        st.subheader("City Compliance Analysis")
        city_compliance = (
            df.groupby("city")
            .agg(
                total_weight=("invoice_weight_mt", "sum"),
                compliant_weight=("invoice_weight_mt", lambda x: x[df.loc[x.index, "in_compliance"]].sum()),
                flagged_weight=("invoice_weight_mt", lambda x: x[df.loc[x.index, "flagged"]].sum()),
            )
            .reset_index()
        )
        city_compliance = city_compliance[city_compliance["city"] != ""]  # Filter out empty cities
        city_compliance["compliance_rate"] = (
            city_compliance["compliant_weight"] / (city_compliance["total_weight"] + 1e-6) * 100
        ).round(1)
        city_compliance = city_compliance.sort_values(by="total_weight", ascending=False).head(10)
        
        if not city_compliance.empty:
            fig = px.bar(
                city_compliance,
                x="city",
                y=["compliant_weight", "flagged_weight"],
                title="Weight Distribution by City: Compliant vs Flagged (MT)",
                labels={"value": "Weight (MT)", "variable": "Status"},
                color_discrete_map={"compliant_weight": "green", "flagged_weight": "red"},
            )
            fig.update_layout(xaxis_tickangle=45)
            st.plotly_chart(fig, use_container_width=True)
            
            # City compliance table
            st.subheader("Top Cities by Weight and Compliance")
            city_display = city_compliance[["city", "total_weight", "compliance_rate"]].copy()
            city_display.columns = ["City", "Total Weight (MT)", "Compliance Rate (%)"]
