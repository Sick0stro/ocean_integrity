...
def normalize_city_name(city):
    """Normalize city names to handle variations and inconsistencies."""
    ...
    if city_str in city_mappings:
        return city_mappings[city_str]
    
    # Check for partial matches
    for key, value in city_mappings.items():
        if key.upper() in city_str or city_str in key.upper():
            return value
...
def run_pipeline_to_single_csv(input_file, output_dir="."):
    ...
    for _, row in df.iterrows():
        ...
        if doc_type == "invoice":
            ...
            ewaybills.append({**base,
                ...
                "weight": d.get("weight"), "weight_kg": normalize_weight_decimal_rule(d.get("weight")),
                "city": normalize_city_name(d.get("city")),
            })
    ...
    final_df = pd.concat(out_frames, ignore_index=True) if out_frames else pd.DataFrame(columns=[
        ...
        "ship_to_country_code","vehicle_number","generated_date","created_at","city",
    ...
    col_order = ["user_id","user_name","invoice_file_url","ewaybill_file_url","invoice_weight_mt",
        ...
        "generated_date","created_at","city","flagged","flag_reason","flagged_pair_value","in_compliance"]
    for c in col_order:
