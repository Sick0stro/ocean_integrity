...
def clean_vehicle_token(s):
    if not s: return ""
    s = s.upper().replace("O","0").replace("I","1").replace("L","1").replace("S","5")
    return re.sub(r"[^A-Z0-9]", "", s)

def normalize_vehicle_no(v):
    if not v: return ""
    tokens = re.split(r"[,&/]", str(v))
    plates=[]
    for t in tokens:
        t=t.strip()
        if not t or not any(ch.isalpha() for ch in t) or not any(ch.isdigit() for ch in t):
            continue
...
def normalize_eway_bill_no(s): return only_alnum_upper(s)

def normalize_date(dt):
    if not dt or (isinstance(dt,float) and math.isnan(dt)): return ""
    s=str(dt).strip()
...
COMMON_SUFFIXES=["PRIVATE LIMITED","PVT LTD","PVT. LTD.","LTD","LIMITED",
...
def recompute_matches(inv_norm,eway_norm,user_id,user_name):
    ...
    for _,e in eway_norm.iterrows():
        ...
        for inv in candidates:
            ...
            comp_match=company_fuzzy_match(e.get("ship_from_company_name",""),inv.get("bill_from_company_name",""))
            if veh_match or comp_match: filtered.append(inv)
        pool=filtered if filtered else candidates
        if not pool: continue
        best=pool[0]
        row={
            "user_id":user_id,"user_name":user_name,
            "invoice_file_url":best.get("file_url"),"ewaybill_file_url":e.get("file_url"),
            "invoice_weight_mt":int(round(best.get("weight_kg")/1000.0)) if pd.notna(best.get("weight_kg")) else 0,
            ...
            "plastic_type":normalize_plastic_type(best.get("plastic_type","")),
            ...
            "flagged":"no","flag_reason":"","flagged_pair_value":"",
            "in_compliance":"yes"
        }
        rows.append(row)
...
def run_pipeline_to_single_csv(input_file,output_dir="."):
    ...
    for _,row in df.iterrows():
        ...
        base={"id":row["id"],"user_id":user_id,"user_name":user_name,
              "file_url":row.get("file_url"),"raw_json":row["raw_json"],
        ...
        if doc_type=="invoice":
            invoices.append({**base,
                ...
                "weight":d.get("weight"),
                "weight_kg":normalize_weight_decimal_rule(d.get("weight"))})
    ...
    print(f"\n✅ Dashboard input saved to: {os.path.abspath(out_path)}  shape={final_df.shape}")
...
if __name__=="__main__":
    ...
    print(f"Dashboard input saved to: {path}  shape={shape}")
