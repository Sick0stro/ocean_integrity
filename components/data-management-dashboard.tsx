...
import { FileText, RotateCcw, Trash2, ExternalLink, RefreshCw, ChevronDown } from "lucide-react"
...
const getRetryOptions = (item: PDFData): RetryOption[] => {
  ...
  if (item.source_table === 'temp_documents') {
    ...
    if (item.status === 'successfully_parsed' && item.total_pages && item.total_pages > 1) {
      options.push({
        action: 'auto',
  ...
  if (item.source_table === 'single_documents') {
    ...
    if (item.last_error?.includes('duplicate') || item.last_error?.includes('409')) {
      options.push({
  ...
  if (options.length === 0 || item.status === 'failed_preprocessing' || item.status === 'failed_ai_processing') {
    options.push({
      action: 'auto',
      label: 'Smart Retry',
...
const getStatusBadge = (status: ProcessingStatus) => {
  const statusConfig = {
    failed_preprocessing: { label: "Failed Preprocessing", variant: "destructive" as const },
    failed_ai_processing: { label: "Failed AI Processing", variant: "secondary" as const },
    successfully_parsed: { label: "Successfully Parsed", variant: "default" as const },
    successfully_pushed_blockchain: { label: "Pushed to Blockchain", variant: "outline" as const },
  }
...
export function DataManagementDashboard({ session }: DataManagementDashboardProps) {
  const [data, setData] = useState<PDFData[]>([])
  const [selectedItem, setSelectedItem] = useState<PDFData | null>(null)
  const [loading, setLoading] = useState(false)
  const [retryDialogOpen, setRetryDialogOpen] = useState(false)
  const [itemToRetry, setItemToRetry] = useState<PDFData | null>(null)
  ...
  const fetchUnprocessedDocuments = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const transformedData: PDFData[] = result.data.map((item: {
          id: string
          original_filename?: string
          pdf_path?: string
          status?: string
          ...
          pdfUrl: item.pdf_path ? `/api/serve-document/${item.id}` : '/placeholder.pdf',
          status: mapDatabaseStatusToUIStatus(item.status || 'uploaded', item.source_table || 'single_documents'),
          data: {
            metadata: {
            ...
            extractedText: item.last_error ? `Error: ${item.last_error}` : undefined,
            processingLogs: [
              `📊 Source Table: ${item.source_table}`,
              `📋 Status: ${item.status || 'uploaded'}`,
              `🔄 Retry Count: ${item.retry_count || 0}`,
              item.page_number ? `📄 Page: ${item.page_number}/${item.total_pages}` : '',
              item.file_size ? `📦 File Size: ${(item.file_size / 1024).toFixed(1)} KB` : '',
              item.last_error
                ? `❌ Last Error: ${item.last_error}`
                : '✅ No errors recorded',
              `🎯 Processing Stage: ${item.source_table === 'temp_documents' ? 'PDF Preprocessing' : 'AI Analysis'}`,
            ].filter(Boolean),
          },
    ...
    try {
      ...
      if (response.ok) {
        ...
        const actionMessages: Record<string, string> = {
          ...
          'force_preprocessing': 'Preprocessing retry initiated - PDF will be re-split and processed',
          'force_ai': 'AI processing retry initiated - document will be re-analyzed',
          'skip_duplicate': 'Document marked as duplicate and skipped',
        }

        alert(actionMessages[action] || result.message || 'Retry initiated successfully')
      ...
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FileText className="h-5 w-5" />Unprocessed Documents
          </CardTitle>
          <CardDescription>
            Documents that are stuck in processing or failed - ready for cleanup or retry
                  ...
                  {data.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center py-8 text-muted-foreground">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-sm">All your documents have been processed successfully!</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
                    data.map((item) => (
                      <TableRow key={item.id}>
                            ...
                            <Dialog>
                              ...
                              <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                                <DialogHeader>
                                  ...
                                  <DialogDescription>Detailed information about the PDF processing</DialogDescription>
                                </DialogHeader>
                                {selectedItem && (
                                  <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                      ...
                                      <div>
                                        <h4 className="font-semibold mb-2">PDF URL</h4>
                                    ...
                                    {selectedItem.data.extractedText && (
                                      <div>
                                        ...
                                        <div className="bg-muted p-3 rounded-md max-h-40 overflow-y-auto">
                                          <p className="text-sm">{selectedItem.data.extractedText}</p>
                                        </div>
                                      </div>
                                    )}
                                    ...
                                    {selectedItem.data.processingLogs && (
                                      <div>
                                        ...
                                        <div className="space-y-1">
                                          {selectedItem.data.processingLogs.map((log, index) => (
                        ...
                        <TableCell className="text-muted-foreground">{formatDate(item.uploadedAt)}</TableCell>
                              ...
                              if (retryOptions.length === 1) {
                                ...
                                return (
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => handleRetry(item.id, option.action)}
                                    className="h-8 w-8 p-0"
      ...
      <Dialog open={retryDialogOpen} onOpenChange={setRetryDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Choose the best retry strategy for &ldquo;{itemToRetry?.fileName}&rdquo;
            </DialogDescription>
          </DialogHeader>
          {itemToRetry && (
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the PDF document and all associated data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
