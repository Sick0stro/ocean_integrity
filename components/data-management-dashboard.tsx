...
const mapDatabaseStatusToUIStatus = (
  ...
  if (sourceTable === 'single_documents') {
    ...
    if (dbStatus === 'uploaded') return 'ready_for_ai'; // ✅ Ready for AI processing (not failed!)
    return 'failed_ai_processing';
  }
  ...
  const fetchUnprocessedDocuments = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const transformedData: PDFData[] = result.data.map(
          (item: {
            id: string;
            original_filename?: string;
            pdf_path?: string;
        ...
        const readyCount = result.data.filter(
          (item: { doc_type?: string; status?: string }) =>
            item.doc_type === 'ready_for_ai' && item.status !== 'processing'
        ).length;

        const processingCount = result.data.filter(
          (item: { status?: string }) => item.status === 'processing'
        ).length;

        const failedCount = result.data.filter(
          (item: { doc_type?: string }) => item.doc_type === 'failed'
        ).length;

        setReadyForAICount(readyCount);
  ...
  useEffect(() => {
    if (!isProcessing) {
      ...
      const timer = setTimeout(() => {
        fetchUnprocessedDocuments();
      }, 2000); // 2 second delay to allow DB updates to complete
            ...
            <div className='flex-1 min-w-0'>
              ...
              <CardDescription>
                {data.length === 0 ? (
                  ...
                  <>
                    {readyForAICount > 0 &&
                      `${readyForAICount} ready for AI processing`}
                    {readyForAICount > 0 && processingCount > 0 && ' • '}
                    {processingCount > 0 &&
                      `${processingCount} currently processing`}
                    {(readyForAICount > 0 || processingCount > 0) &&
                      failedCount > 0 &&
                      ' • '}
                    {failedCount > 0 && `${failedCount} failed (needs retry)`}
                  </>
                )}
              </CardDescription>
            </div>

            {/* Process Documents Button - shown only when there are ready documents */}
            {readyForAICount > 0 && onProcessDocuments && (
              <Button
                onClick={onProcessDocuments}
                disabled={isProcessing}
                className='bg-green-600 hover:bg-green-700 text-white flex-shrink-0 whitespace-nowrap'
                size='lg'
              >
                {isProcessing ? (
                  <>
                  ...
                  <>
                    ⚡ Process {readyForAICount} Document
                    {readyForAICount > 1 ? 's' : ''}
                  </>
                )}
              </Button>
                      ...
                      <TableRow key={item.id}>
                        ...
                        <TableCell className='text-right'>
                          <div className='flex items-center justify-end gap-1'>
                            {(() => {
                              // Don't show button for processing, successfully processed, or blockchain docs
                              const canProcess = !(
                              ...
                              if (!canProcess) {
                                // Hide button entirely for processing files
                              ...
                              const isReadyForAI =
                                item.status === 'ready_for_ai';

                              // For ready_for_ai, show "Process" button
                              if (isReadyForAI) {
                                return (
                                  <Button
                                    variant='default'
                                    ...
                                    className='h-8 w-auto px-3 bg-green-600 hover:bg-green-700 text-white'
